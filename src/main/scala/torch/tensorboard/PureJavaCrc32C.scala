 ...
 */
object PureJavaCrc32C { // CRC polynomial tables generated by:
...
/** Create a new PureJavaCrc32 object. */
  ...
  override def update(b: Array[Byte], offset: Int, length: Int): Unit = {
    ...
    while (len > 7) {
      ...
      localCrc = (PureJavaCrc32C.T8_7(c0 & 0xff) ^ PureJavaCrc32C.T8_6(c1 & 0xff)) ^ (PureJavaCrc32C.T8_5(c2 & 0xff) ^ PureJavaCrc32C.T8_4(c3 & 0xff))
      localCrc ^= (PureJavaCrc32C.T8_3(b({
        off += 1;
        off - 1
      }) & 0xff) ^ PureJavaCrc32C.T8_2(b({
        off += 1;
        off - 1
      }) & 0xff)) ^ (PureJavaCrc32C.T8_1(b({
        off += 1;
        off - 1
      }) & 0xff) ^ PureJavaCrc32C.T8_0(b({
        off += 1;
        off - 1
      }) & 0xff))
      len -= 8
    ...
    while (len > 0) {
      localCrc = (localCrc >>> 8) ^ PureJavaCrc32C.T8_0((localCrc ^ b({
        off += 1;
        off - 1
      })) & 0xff)
      len -= 1
...
//  }
  ...
  override final def update(b: Int): Unit = {
    crc = (crc >>> 8) ^ PureJavaCrc32C.T8_0((crc ^ b) & 0xff)
  }
}